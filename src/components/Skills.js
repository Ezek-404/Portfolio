  ...
  return (  
    <motion.div className='flex items-center justify-center rounded-full font-semibold bg-dark text-light py-3 px-6
     ...
     lg:py-2 lg:px-4 md:text-sm md:py-1.5 md:px-3 xs:bg-transparent xs:dark:bg-transparent xs:text-dark xs:dark:text-light xs:font-bold' 
            whileHover={{scale:1.05}} initial={{x:0, y:0}} whileInView={{x:x, y:y, transition:{duration:1.05}}} viewport={{once: true}}>
                {name}
            </motion.div>
  )
}
  ...
  return (
    <>
        ...
        lg:bg-circularLightLg lg:dark:bg-circularDarkLg md:bg-circularLightMd md:dark:bg-circularDarkMd sm:bg-circularLightSm sm:dark:bg-circularDarkSm'>
            ...
            <Skill name="ReactJS" x="10vw" y="-17vw" /> 
            <Skill name="MySQL" x="-18vw" y="15vw" /> 
            <Skill name="PHP" x="6vw" y="14vw" /> 
        </div>
    </>
  )
}
